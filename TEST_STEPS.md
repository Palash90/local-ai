...
- TTS disk cache (`~/local-ai-files/tts_cache/`, secondary via `TTS_CACHE_SECONDARY_DIR` in `.env`, unset = skipped): first play writes `<sha256>.wav/.mp3`; replay reads disk (~ms, survives restarts); files moved by hand to secondary still play with no re-synth; corrupt entries are dropped and re-synthesized; primary pruned to `TTS_CACHE_MAX_BYTES` (1 GB default). Chunked long-form synthesis (up to 8000 chars authenticated, 2000 chars public) splits at sentence boundaries into <=1800 char chunks and concatenates audio seamlesssly. Session deletion and share revoke (with purge=1) eagerly delete orphaned audio files from both primary and secondary cache directories.
