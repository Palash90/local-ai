...
- `POST /api/tts` `{text}` → audio bytes. Routing: `[xx]` tag or script detect → `en` (lessac-high), `es` (claude-high), `hi`/`bn`/`te` (Piper, process-cached, lock-serialized); `kn` → edge-tts MP3. Markdown is cleaned first (`**`, links/URLs, code blocks, `|` separators never spoken; `Word:` → `Word,`). Repeat identical request → `cache hit`, ~ms. Missing Piper voice file → clean 5xx (no hang). Voices live in `~/.piper_voices/` (es/en-high are downloads, not in repo).
