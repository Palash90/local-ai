...
# image_active rule). Guarded by ``_chat_generating_lock``.
_chat_generating = 0
# Per-lane version of the above (mode → count) so callers can wait on a
# specific lane instead of the whole box. Kept in sync under the same lock.
